...
module.exports = function (instream, outstream, done) {
  ...
  function summarise () {
    for (var label of Object.keys(results)) {
      ...
      results[label].duration_s = results[label].time_end - results[label].time_start;
      results[label].throughput = +(results[label].count / (results[label].duration_s / 1000)).toFixed(2);
