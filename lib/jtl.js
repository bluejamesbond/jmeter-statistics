...
module.exports = function (instream, outstream, done) {
  ...
  function summarise () {
    for (var label of Object.keys(results)) {
      ...
      results[label]["throughput"] = +((1000 * results[label].count) / results[label].duration).toFixed(2);
  ...
  function end () {
    summarise();
    var keys = ["label", "count", "minBytes", "maxBytes", "averageBytes", "duration", "throughput", "min", "max", "average", "%50 line", "%90 line", "%99 line", "% error", "apdex", "satisfied", "tolerating", "frustrated"];
