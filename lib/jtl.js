...
module.exports = function (instream, outstream, done) {
  ...
  function summarise () {
    for (var label of Object.keys(results)) {
      ...
      results[label].averageBytes = +(results[label].totalBytes / results[label].samples).toFixed(2);
  ...
  function end () {
    summarise();
    var keys = ["label", "samples", "minBytes", "maxBytes", "averageBytes", "duration", "throughput (rps)", "min", "max", "average", "%50 line", "%90 line", "%99 line", "% error", "apdex", "satisfied", "tolerating", "frustrated"];
