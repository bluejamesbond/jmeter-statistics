...
module.exports = function (instream, outstream, done) {
  ...
  function data (line) {
    if (line && line.indexOf('timeStamp,') < 0) {
      ...
      results[row.label].total += +row.elapsed;
  ...
  function summarise () {
    for (var label of Object.keys(results)) {
      results[label].average = +(results[label].total / results[label].samples).toFixed(2);
      results[label].duration = results[label].max - results[label].min;
  ...
  function end () {
    summarise();
    var keys = ["label", "samples", "throughput (rps)", "average", "max", "min", "%50 line", "%90 line", "%99 line", "% error", "apdex", "satisfied", "tolerating", "frustrated"];
