...
module.exports = function (instream, outstream, done) {
  ...
  function addLabel (label) {
    if (!results[label]) {
      results[label] = {
        ...
        maxBytes: Number.MIN_VALUE,
        minBytes: Number.MAX_VALUE,
        totalBytes: 0,
        duration: 0,
        elapsedTime: [],
        satisfied: 0,
  ...
  function data (line) {
    if (line && line.indexOf('timeStamp,') < 0) {
      ...
      results[row.label].minBytes = Math.min(results[row.label].minBytes, +row.bytes);
      results[row.label].maxBytes = Math.max(results[row.label].maxBytes, +row.bytes);
  ...
  function summarise () {
    for (var label of Object.keys(results)) {
      results[label].average = +(results[label].duration / results[label].count).toFixed(2);
      results[label].averageBytes = +(results[label].totalBytes / results[label].count).toFixed(2);
      
      results[label]["throughput (rps)"] = +((1000 * results[label].count) / results[label].duration).toFixed(2);
      results[label].apdex = +((results[label].satisfied + results[label].tolerating / 2) / results[label].count).toFixed(2);
  ...
  function end () {
    summarise();
    var keys = ["label", "count", "minBytes", "maxBytes", "averageBytes", "duration", "throughput (rps)", "min", "max", "average", "%50 line", "%90 line", "%99 line", "% error", "apdex", "satisfied", "tolerating", "frustrated"];
