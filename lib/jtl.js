...
module.exports = function (instream, outstream, done) {
  ...
  function data (line) {
    if (line && line.indexOf('timeStamp,') < 0) {
      ...
      results[row.label].responsetime_max = Math.max(results[row.label].max, +row.elapsed);
      results[row.label].responsetime_min = Math.min(results[row.label].min, +row.elapsed);
  ...
  function summarise () {
    for (var label of Object.keys(results)) {
      results[label].responsetime_avg = +(results[label].duration / results[label].count).toFixed(2);
